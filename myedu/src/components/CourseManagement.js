...
import { PlusCircle, Edit, Trash2, X, AlertCircle, CheckCircle, Info, Layers, ListTodo, Clipboard } from 'lucide-react';
...
// Replaced Quill editor with `CourseEditor` (simple HTML source + preview)
...
// --- NEW: Topic Editor Modal Component ---
...
const CourseManagement = () => {
    ...
    const fetchCourses = useCallback(async () => {
        ...
        try {
            ...
            if (response.ok) {
                // Check if modules is populated using forEach
                data.forEach(course => {
    ...
    const startEditingCourse = (course) => {
        ...
        setModules(course.modules || []); // initialise modules state for the course
    ...
    const handleUpdateCourse = async (e) => {
        ...
        try {
            ...
            if (response.status === 401 || response.status === 403) {  // by myself
                setFormMessage({ type: 'error', text: 'Authentication failed. Please log in again.' });
                logout(); 
                setIsLoading(false);
    ...
    const handleDeleteCourse = async () => {
        ...
        const handleAddModule = async (courseId) => {
            ...
            setIsLoading(true);
            try {
                // Find the specific course object locally to get the correct module array length for `newOrder`
                ...
                if (response.ok) {
                    ...
                    const newModuleData = { ...data, topics: [] }; // The new module object
                    
                    // 2. Update the main 'courses' list
                    setCourses(prevCourses => prevCourses.map(c => {
                        if (c._id === courseId) {
                            // Create a new modules array by spreading the existing populated array and appending the new populated module data
                            const updatedModules = [...(c.modules || []), newModuleData].sort((a, b) => a.order - b.order);
                            // Add the module reference (ID) to the parent course's module array
                    ...
                    await fetchCourses(); // Refresh courses to ensure consistency
        ...
        const handleDeleteModule = async (moduleId, title) => {
            ...
            try {
                ...
                // Find the course ID by searching the main courses list (safest way if local modules state isn't guaranteed)
                ...
                if (response.ok) {
                    ...
                    // Get the courseId of the module being deleted (assuming the current module state is the one being edited)
                    // const courseId = modules.find(m => m._id === moduleId)?.courseId;
                    
                    // // 2. Update the main 'courses' list
                    // setCourses(prevCourses => prevCourses.map(c => {
                    //     if (c._id === courseId) {
                    //         // Filter the module out of the parent course's module array
                    //         const updatedModules = (c.modules || []).filter(m => m._id !== moduleId).sort((a, b) => a.order - b.order);
                    //         return { ...c, modules: updatedModules };
                    //     }
                    //     return c;
                    // }));
                ...
                // setIsDeleting(false);  // why not like prviously?
                // setCourseToDelete(null);
                setIsLoading(false);
    ...
    try {
        ...
        if (response.ok) {
            const newTopicData = { ...data }; // This is an OBJECT (the new topic)
            const courseId = currentModule.courseId;

            // 1. Update local modules state
            setModules(prev => prev.map(m => {
                if (m._id === moduleId) {
                    // Create the new topics array, append newTopicData, and sort it.
        ...
        const handleSaveTopicContent = async (updatedTopicData) => {
            ...
            const courseId = modules.find(m => m._id === moduleId)?.courseId; // Get parent course ID
        ...
        const handleDeleteTopic = async (topicId, moduleId, title) => {
            ...
            // ✅ This logic is correct for finding the parent course ID from the local state
        const courseId = courses.find(c => c.modules.some(m => m._id === moduleId))?._id; 
    
        // The rest of the API call and state updates are structurally sound, 
        // assuming the correct courseId is found. 
        // If courseId is NULL/UNDEFINED, the update will fail silently on the frontend.
        if (!courseId) {
        ...
        // 2. Filter by search term on the remaining list
        ...
        const renderCourseContentManagement = () => (
            <div className="mt-8 p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                <h3 className="admin-section-title"><Layers size={20} /> Course Structure & Content Management</h3>
                ...
                {filteredCoursesForContent.map(course => (
                    <div key={course._id} className="mb-8 p-4 border border-teal-500 dark:border-teal-700 rounded-lg">
                        <h4 className="text-xl font-bold text-teal-600 dark:text-teal-400 mb-4">{course.title}</h4>
                        <button onClick={() => handleAddModule(course._id)} className="admin-button-secondary mb-4">
                            <PlusCircle size={16} className="icon-mr" /> Add Module to {course.title}
                        </button>
                        
                        {(course.modules || []).sort((a, b) => a.order - b.order).map(module => (
                            <div key={module._id} className="ml-4 pl-4 border-l-4 border-gray-300 dark:border-gray-600 mb-6">
                                <div className="flex justify-between items-center bg-gray-100 dark:bg-gray-700 p-3 rounded-t-md">
                                    <h5 className="text-lg font-semibold text-gray-900 dark:text-gray-200">
                                        <ListTodo size={18} className="inline-block icon-mr" /> Module {module.order}: {module.title}
                                    </h5>
                                    <div className="flex gap-2">
                                        <button onClick={() => handleAddTopic(module._id)} className="admin-action-button edit-button bg-green-500 hover:bg-green-600 text-white" title="Add Topic">
                                            <PlusCircle size={16} />
                                        </button>
                                        <button onClick={() => handleDeleteModule(module._id, module.title)} className="admin-action-button delete-button" title="Delete Module">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </div>
                                ...
                                {(module.topics || []).sort((a, b) => a.order - b.order).map(topic => (
                                    <div key={topic._id} className="flex justify-between items-center p-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                                        <span className="text-gray-700 dark:text-gray-300">Topic {topic.order}: {topic.title}</span>
                                        <div className="flex gap-2">
                                            <button onClick={() => handleOpenTopicEditor(module, topic)} onMouseEnter={() => { import('./CourseEditorEditorPanel'); }} className="admin-action-button edit-button" title="Edit Content/Notes/Quiz">
                                                <Edit size={16} />
                                            </button>
                                            <button onClick={() => handleDeleteTopic(topic._id, module._id, topic.title)} className="admin-action-button delete-button" title="Delete Topic">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    </div>
                                ))}
                                {(module.topics || []).length === 0 && <p className="p-3 text-sm text-gray-500 dark:text-gray-400">No topics in this module. Click '+' to add one.</p>}
                            </div>
                        ))}
                        {(course.modules || []).length === 0 && <p className="text-center text-lg text-gray-500 dark:text-gray-400">No modules created for this course yet.</p>}     
                    </div>
                ))}
            </div>
        );
    ...
    return (
        <>
            ...
            <form onSubmit={handleAddCourse} className="admin-form-container">
                <div className="form-group"><label htmlFor="newCourseTitle" className="form-label">Title</label><input type="text" id="newCourseTitle" name="title" value={newCourse.title} onChange={(e) => setNewCourse({ ...newCourse, title: e.target.value })} required className="form-input" /></div>
                <div className="form-group"><label htmlFor="newCourseDescription" className="form-label">Description</label><textarea id="newCourseDescription" name="description" value={newCourse.description} onChange={(e) => setNewCourse({ ...newCourse, description: e.target.value })} rows="3" required className="form-textarea"></textarea></div>
                <div className="form-group"><label htmlFor="newCourseType" className="form-label">Type</label><select id="newCourseType" name="type" value={newCourse.type} onChange={(e) => setNewCourse({ ...newCourse, type: e.target.value, price: e.target.value === 'free' ? 'Free' : '' })} className="form-select"><option value="paid">Paid</option><option value="free">Free</option></select></div>
                <div className="form-group"><label htmlFor="newCoursePrice" className="form-label">Price (e.g., ₹9,999 or Free)</label><input type="text" id="newCoursePrice" name="price" value={newCourse.price} onChange={(e) => setNewCourse({ ...newCourse, price: e.target.value })} required={newCourse.type === 'paid'} disabled={newCourse.type === 'free'} className={`form-input ${newCourse.type === 'free' ? 'disabled' : ''}`} /></div>
                <div className="form-group"><label htmlFor="newCourseImageUrl" className="form-label">Image URL</label><input type="text" id="newCourseImageUrl" name="imageUrl" value={newCourse.imageUrl} onChange={(e) => setNewCourse({ ...newCourse, imageUrl: e.target.value })} className="form-input" placeholder="e.g., https://example.com/course.jpg" /></div>
                <div className="form-group"><label htmlFor="newCourseRating" className="form-label">Rating (0-5)</label><input type="number" id="newCourseRating" name="rating" value={newCourse.rating} onChange={(e) => setNewCourse({ ...newCourse, rating: parseFloat(e.target.value) || 0 })} min="0" max="5" step="0.1" className="form-input" /></div>
                <div className="form-group"><label htmlFor="newCourseStatus" className="form-label">Status</label><select id="newCourseStatus" name="status" value={newCourse.status} onChange={(e) => setNewCourse({ ...newCourse, status: e.target.value })} className="form-select"><option value="running">Running</option><option value="upcoming">Upcoming</option></select></div>
                <button type="submit" disabled={isLoading} className="form-submit-button">{isLoading ? 'Adding...' : <><PlusCircle size={20} className="icon-mr" /> Add Course</>}</button>
            </form>
            <h3 className="admin-section-title"><Info size={20} /> Existing Courses</h3>
            {isLoading ? (<p className="text-center text-gray-500 dark:text-gray-400">Loading courses...</p>) : courses.length === 0 ? (<p className="admin-message-info">No courses found. Add a new course above!</p>) : (
                        ...
                        <tbody>
                            {courses.map((course) => (
                                <tr key={course._id} className="admin-table-tr">
                                    ...
                                    {/* ADD THIS LINE for Modules Count */}
                                    ...
                                    <button onClick={() => startEditingCourse(course)} onMouseEnter={() => { import('./CourseEditorEditorPanel'); }} title="Edit" className="admin-action-button edit-button"><Edit size={18} /></button>
                                    <button onClick={() => confirmDeleteCourse(course._id)} title="Delete" className="admin-action-button delete-button"><Trash2 size={18} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {isEditModalOpen && editingCourse && (
                <div className="modal-overlay">
                    <div className="modal-content-box">
                        <div className="modal-header"><h3 className="modal-title">Edit Course: {editingCourse.title}</h3><button onClick={() => setIsEditModalOpen(false)} className="modal-close-button"><X size={24} /></button></div>
                        <form onSubmit={handleUpdateCourse}>
                            ...
                            <div className="form-group"><label htmlFor="editImageUrl" className="form-label">Image URL</label><input type="text" id="editImageUrl" name="imageUrl" value={editingCourse.imageUrl} onChange={handleEditChange} className="form-input" /></div>
            ...
            <ConfirmationModal show={isDeleting} title="Confirm Deletion" message="Are you sure you want to delete this course? This action cannot be undone." onConfirm={handleDeleteCourse} onCancel={() => setIsDeleting(false)} />
                  ...
                  {/* NEW: Dedicated Section for Module/Topic Management */}
            {renderCourseContentManagement()}
            
            {/* NEW: Topic Content Editor Modal */}
            <TopicEditorModal
