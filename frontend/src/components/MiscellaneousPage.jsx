...
import { ChevronRight, ChevronLeft, Layers, BookOpen, ArrowLeft, ArrowRight, Search, SlidersHorizontal } from 'lucide-react';
...
const MiscellaneousPage = () => {
    ...
    const fetchSections = useCallback(async () => {
        ...
        setError('');
        try {
            const res = await fetch(`${API_BASE_URL}/api/public/sections?type=miscellaneous`);
            if (res.ok) {
            ...
            console.error('Error fetching miscellaneous sections:', err);
    ...
    const handleSelectSection = useCallback(async (section, targetNoteId = null) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data && data.length > 0) {
                    const matchedNote = targetNoteId ? data.find(n => n._id === targetNoteId) : null;
                    setSelectedNote(matchedNote || data[0]);
                }
            } else {
    ...
    useEffect(() => {
        if (location.state && location.state.sectionId && sections.length > 0) {
            const section = sections.find(s => s._id === location.state.sectionId);
            if (section) {
                handleSelectSection(section, location.state.noteId);
            }
        }
    }, [location.state, sections, handleSelectSection]);
    ...
    if (error) {
        return (
            <div className="container mx-auto px-4 pt-6 pb-12 text-center text-red-500 min-h-[calc(100vh-7rem)] flex flex-col justify-center items-center">
                ...
                <button onClick={fetchSections} className="px-6 py-2 rounded-xl bg-purple-650 text-white font-semibold shadow hover:opacity-90">Try Again</button>
            </div>
        );
    }
    ...
    return (
        <div className="bg-transparent min-h-screen text-gray-900 dark:text-white transition-colors duration-500">
            <main className="container mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12">
                {!selectedSection ? (
                    <>
                        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-center bg-gradient-to-r from-purple-500 via-indigo-400 to-violet-500 dark:from-purple-400 dark:via-indigo-300 dark:to-violet-400 bg-clip-text text-transparent mb-6">
                            Miscellaneous Topics
                            <span className="block w-24 h-2 bg-gradient-to-r from-purple-500 to-violet-500 mx-auto mt-4 rounded-full"></span>
                        </h1>
                        <p className="text-center text-gray-600 dark:text-gray-300 max-w-2xl mx-auto mb-8 text-sm sm:text-base">
                            Explore a curated pool of topics — real-time discussions, general knowledge articles, and community-contributed learning content.
                        </p>
                        ...
                        <div className="max-w-4xl mx-auto mb-10 bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-2xl p-4 sm:p-5 shadow-md backdrop-blur flex flex-col sm:flex-row gap-4 items-center">
                            <div className="relative flex-1 w-full">
                                ...
                                <input
                                    type="text"
                                    placeholder="Search miscellaneous topics by keyword..."
                                    value={searchQuery}
                        ...
                        {paginatedSections.length === 0 ? (
                            <div className="text-center py-16 text-gray-500 dark:text-gray-400 font-semibold bg-white dark:bg-white/5 rounded-2xl border border-gray-300 dark:border-white/10 max-w-lg mx-auto">
                                <Layers size={48} className="mx-auto mb-4 text-purple-400 dark:text-purple-300 opacity-60" />
                                <p className="text-lg">No topics matched your search.</p>
                            </div>
                        ) : (
                            <>
                                    ...
                                    {paginatedSections.map(section => (
                                        ...
                                        >
                                            <div className="relative h-36 overflow-hidden bg-gray-100 dark:bg-white/10">
                                                <img
                                                    src={section.imageUrl || 'https://placehold.co/600x400/7c3aed/ffffff?text=Topic'}
                                                    alt={section.title}
                                            ...
                                            <div className="p-4 flex-1 flex flex-col justify-between">
                                                <p className="text-gray-600 dark:text-gray-300 text-xs mb-3 line-clamp-3">
                                                    {section.description || 'Explore this collection of miscellaneous topics and learning content.'}
                                                </p>
                                                <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-150 dark:border-white/5">
                                                    <span className="text-[11px] font-semibold text-purple-600 dark:text-purple-300 group-hover:underline flex items-center gap-1">
                                                        Explore Topics <ChevronRight size={12} />
                                                    </span>
                                                    <Layers size={16} className="text-gray-400 dark:text-white/50 group-hover:rotate-12 transition-transform duration-300" />
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                    ...
                    <div>
                        ...
                        {notesLoading ? (
                            ...
                            <div className="text-center py-12 text-gray-500 dark:text-gray-400 font-semibold bg-white dark:bg-white/5 rounded-2xl border border-gray-300 dark:border-white/10">
                                This topic does not have any content assigned yet. Check back soon!
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                                {/* Left Side Topics Panel */}
                                <div className={`lg:col-span-3 bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-2xl p-4 sm:p-6 shadow-md backdrop-blur lg:sticky lg:top-24 transition-all duration-300 ${isSidebarOpen ? 'block' : 'hidden'}`}>
                                    <h3 className="text-md font-bold text-gray-800 dark:text-white/80 border-b border-gray-200 dark:border-white/10 pb-3 mb-4 flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2">
                                            <BookOpen size={18} className="text-purple-500" /> Topics List
                                        </div>
                                        <button 
                                        ...
                                        {notes.map((note, idx) => {
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className={`p-3.5 rounded-xl cursor-pointer transition-all duration-200 border text-left ${isActive ? 'bg-purple-50 dark:bg-purple-950/20 border-purple-300 dark:border-purple-800/50 text-purple-800 dark:text-purple-300 font-bold shadow-sm' : 'bg-transparent border-transparent text-gray-700 dark:text-white/70 hover:bg-gray-50 dark:hover:bg-white/5 hover:border-gray-200 dark:hover:border-white/10'}`}
                                                >
                                                ...
                                                <div className="flex items-center">
                                                    {!isSidebarOpen && (
                                                        <button 
                                                            ...
                                                            className="p-2 rounded-xl bg-purple-50 dark:bg-purple-950/20 border border-purple-250 dark:border-purple-800/50 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-all mr-3 shadow-sm shrink-0"
                                                    ...
                                                    <div>
                                                        ...
                                                        {selectedNote.subject && <span className="text-xs text-purple-600 dark:text-purple-300 font-bold uppercase tracking-wider block mt-1">{selectedNote.subject}</span>}
                                                    </div>
                                                </div>
