...
import { ChevronRight, ChevronLeft, ArrowLeft, ArrowRight, Cpu, Bookmark, Search, SlidersHorizontal } from 'lucide-react';
...
const SoftwareToolsPage = () => {
    ...
    const fetchSections = useCallback(async () => {
        ...
        setError('');
        try {
            const res = await fetch(`${API_BASE_URL}/api/public/sections?type=software_tool`);
            if (res.ok) {
                const data = await res.json();
                setSections(data || []);
            } else {
                setError('Failed to load software and tools.');
            }
        } catch (err) {
            console.error('Error fetching software and tools sections:', err);
    ...
    return (
        <div className="bg-transparent min-h-screen text-gray-900 dark:text-white transition-colors duration-500">
            <main className="container mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12">
                {!selectedSection ? (
                    <>
                        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-center bg-gradient-to-r from-teal-600 via-sky-500 to-indigo-600 dark:from-teal-400 dark:via-sky-300 dark:to-indigo-400 bg-clip-text text-transparent mb-6">
                            Software & Development Tools
                            <span className="block w-24 h-2 bg-gradient-to-r from-teal-500 to-indigo-500 mx-auto mt-4 rounded-full"></span>
                        </h1>
                        <p className="text-center text-gray-600 dark:text-gray-300 max-w-2xl mx-auto mb-8 text-sm sm:text-base">
                            Guides and recommendations on MUST-have software, extensions, utilities, design resources, and configurations to speed up your coding workflows.
                        </p>
                        ...
                        <div className="max-w-4xl mx-auto mb-10 bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-2xl p-4 sm:p-5 shadow-md backdrop-blur flex flex-col sm:flex-row gap-4 items-center">
                            <div className="relative flex-1 w-full">
                                ...
                                <input
                                    type="text"
                                    placeholder="Search software/tools by title or keyword..."
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-300 dark:border-white/10 bg-gray-50 dark:bg-black/20 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-gray-800 dark:text-white"
                                />
                            </div>
                            <div className="relative w-full sm:w-64">
                                ...
                                <select
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-300 dark:border-white/10 bg-gray-50 dark:bg-black/20 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-gray-800 dark:text-white appearance-none cursor-pointer"
                                >
                        ...
                        {paginatedSections.length === 0 ? (
                            <div className="text-center py-16 text-gray-500 dark:text-gray-400 font-semibold bg-white dark:bg-white/5 rounded-2xl border border-gray-300 dark:border-white/10 max-w-lg mx-auto">
                                No software tools found matching your criteria.
                            </div>
                        ) : (
                            <>
                                    ...
                                    {paginatedSections.map(section => (
                                        ...
                                        >
                                            <div className="relative h-36 overflow-hidden bg-gray-100 dark:bg-white/10">
                                                <img
                                                    src={section.imageUrl || 'https://placehold.co/600x400/cccccc/000000?text=Software+Tools'}
                                                    alt={section.title}
                                                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                                                />
                                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                                                <div className="absolute bottom-3 left-3 right-3">
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-500/80 text-white backdrop-blur-sm mb-1">
                                                        Developer Utilities
                                                    </span>
                                            ...
                                            <div className="p-4 flex-1 flex flex-col justify-between">
                                                <p className="text-gray-600 dark:text-gray-300 text-xs mb-3 line-clamp-3">
                                                    {section.description || 'No description available for this software resource guide.'}
                                                </p>
                                                <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-150 dark:border-white/5">
                                                    <span className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-300 group-hover:underline flex items-center gap-1">
                                                        Explore Lists <ChevronRight size={12} />
                                                    </span>
                                                    <Cpu size={16} className="text-gray-400 dark:text-white/50 group-hover:scale-110 transition-transform duration-300" />
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                    ...
                    <div>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-gray-200 dark:border-white/10">
                            <div className="flex flex-wrap items-center gap-4">
                                ...
                                >
                                    <ArrowLeft size={14} /> Back to Software & Tools
                                </button>
                        ...
                        {notesLoading ? (
                            ...
                            <div className="text-center py-12 text-gray-500 dark:text-gray-400 font-semibold bg-white dark:bg-white/5 rounded-2xl border border-gray-300 dark:border-white/10">
                                This category does not have any software/tool lists assigned yet. Check back soon!
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                                {/* Left Side Tool Lists */}
                                <div className={`lg:col-span-3 bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-2xl p-4 shadow-md backdrop-blur lg:sticky lg:top-24 transition-all duration-300 ${isSidebarOpen ? 'block' : 'hidden'}`}>
                                    <h3 className="text-md font-bold text-gray-800 dark:text-white/80 border-b border-gray-200 dark:border-white/10 pb-3 mb-4 flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2">
                                            <Bookmark size={18} className="text-indigo-500" /> Tool Categories
                                        </div>
                                        <button 
                                        ...
                                        {notes.map(note => {
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className={`p-3 rounded-xl cursor-pointer transition-all duration-200 flex items-center gap-3 border ${isActive ? 'bg-indigo-50 dark:bg-indigo-950/20 border-indigo-300 dark:border-indigo-700/50 text-indigo-800 dark:text-indigo-300 font-semibold' : 'bg-transparent border-transparent text-gray-700 dark:text-white/70 hover:bg-gray-100 dark:hover:bg-white/10'}`}
                                                >
                                                    <Cpu size={16} className={isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'} />
                                ...
                                <div className={`bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-2xl p-6 shadow-md backdrop-blur min-h-[400px] transition-all duration-300 ${isSidebarOpen ? 'lg:col-span-9' : 'lg:col-span-12'}`}>
                                    {selectedNote ? (
                                                ...
                                                <div className="flex items-center">
                                                    {!isSidebarOpen && (
                                                        <button 
                                                            ...
                                                            className="p-2 rounded-xl bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-250 dark:border-indigo-800/50 text-indigo-750 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 transition-all mr-3 shadow-sm shrink-0"
                                                            title="Show Tool Categories"
                                                        >
                                                    ...
                                                    <div>
                                                        ...
                                                        {selectedNote.subject && <span className="text-xs text-indigo-650 dark:text-indigo-300 font-bold uppercase tracking-wider block mt-1">{selectedNote.subject}</span>}
                                                    </div>
                                                </div>
                                        ...
                                        <div className="flex items-center justify-center h-full text-gray-400 font-medium">
                                            Select a tool category list to start reading.
                                        </div>
                                    )}
                                </div>
                            </div>
