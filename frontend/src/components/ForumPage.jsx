...
const Post = ({ post, onToggleReplies, numReplies, isRepliesVisible, isAuthorOfParent, parentSolutionId }) => {
    ...
    const staffHighlightClass = authorRole === 'staff' ? 'bg-yellow-500/10 border-l-4 border-yellow-400/50' : '';

    return (
        <div className={`flex items-start space-x-4 p-6 rounded-2xl transition-all duration-300 border border-white/10 bg-white/5 backdrop-blur ${isSolution ? 'post-card-solution-border' : ''} ${staffHighlightClass}`}>
            <div
            ...
            <div className="flex-grow">
                <div className="flex items-center justify-between mb-2">
                    <p className="text-lg font-semibold text-white flex items-center flex-wrap gap-2">
                        ...
                        {post.category && !post.parentId && (
                            <span className="flex items-center text-xs font-medium bg-white/10 border border-white/10 text-gray-200 rounded-full px-2 py-1">
                                {categoryIcons[post.category]} {post.category}
                            </span>
                        )}
                        {/* Staff badge */}
                        {authorRole === 'staff' && (
                            <span className="ml-2 inline-flex items-center text-xs font-semibold bg-yellow-500/15 border border-yellow-500/20 text-yellow-200 rounded-full px-2 py-0.5">
                        ...
                        {isSolution && (
                            <span className="flex items-center text-xs font-medium bg-[#167468]/10 border border-[#167468]/20 text-[#167468] rounded-full px-2 py-1">
                                <Award size={14} className="mr-1" /> Solution
                            </span>
                        )}
                    </p>
                    <div className="flex flex-col items-end">
                        <span className="text-sm text-gray-400 whitespace-nowrap">{formatDate(post.createdAt)}</span>
                        <span className="text-xs text-gray-400">By: {author && (author.username || (author._id ? String(author._id).slice(0,8) : 'Unknown')) || (post.userId || 'Unknown')}</span>
                    </div>
                </div>
                <div className="markdown-content text-gray-200">
                    {ReactMarkdown && RemarkGfm ? (
                        <ReactMarkdown
                            ...
                            components={{
                                code: CodeBlock,
                                p: ({node, ...props}) => <p {...props} className="text-gray-200 my-2" />,
                                li: ({node, ...props}) => <li {...props} className="text-gray-200 ml-5 list-disc" />,
                                ul: ({node, ...props}) => <ul {...props} className="mb-2" />,
                                ol: ({node, ...props}) => <ol {...props} className="mb-2 list-decimal" />,
                                a: ({node, ...props}) => <a {...props} className="text-[#167468] hover:underline" />,
                                h1: ({node, ...props}) => <h1 {...props} className="text-2xl font-bold text-white mb-2 mt-4" />,
                                h2: ({node, ...props}) => <h2 {...props} className="text-xl font-semibold text-white mb-2 mt-3" />,
                                h3: ({node, ...props}) => <h3 {...props} className="text-lg font-medium text-white mb-1 mt-2" />,
                            }}
                        >
                            {post.content}
                        </ReactMarkdown>
                ...
                {post.tags && post.tags.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                        {post.tags.map((tag, index) => (
                            <span key={index} className="bg-white/10 border border-white/10 text-gray-200 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                                {tag}
                            </span>
                        ))}
                    </div>
                ...
                {!post.parentId && (
                    <div className="mt-4 flex items-center space-x-4">
                        <button
                            ...
                            className={`flex items-center px-3 py-1.5 rounded-full text-sm font-medium transition-colors border border-white/10 ${likedByCurrentUser ? 'bg-[#167468]/20 text-white hover:bg-[#167468]/30' : 'bg-white/5 text-gray-200 hover:bg-white/10'}`}
                        ...
                        <button
                            ...
                            className="flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-white/5 border border-white/10 text-gray-200 hover:bg-white/10 transition-colors"
                            disabled={!currentUser}
                        >
                    ...
                    <div className="mt-4 flex items-center space-x-4">
                        ...
                        {isAuthorOfParent && !parentSolutionId && (
                            <button
                                ...
                                className="flex items-center px-3 py-1.5 rounded-full text-sm font-medium text-white bg-gradient-to-r from-purple-500 to-[#167468] border border-white/10 hover:shadow-lg hover:shadow-[#167468]/30 transition-all"
...
/**
 * Button component for navigating to home page forum premium section
 */
const ViewPlansButton = () => {
    const navigate = useNavigate();
    
    const handleViewPlans = () => {
        navigate('/#forum-premium');
    };
    
    return (
        <button
            onClick={handleViewPlans}
            className="inline-block px-5 py-2.5 rounded-2xl bg-gradient-to-r from-purple-500 to-[#167468] text-white font-medium border border-white/10 hover:shadow-lg hover:shadow-[#167468]/30 transition-all"
        >
            View plans on Home
        </button>
    );
        ...
        <ForumProvider forumHandlers={forumHandlers}>
            ...
            <div className="container mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-3">
                    <header className="bg-white/5 border border-white/10 backdrop-blur rounded-2xl shadow-lg p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center space-x-4">
                            <svg
                                ...
                                className="text-[#167468] w-12 h-12"
                            >
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            <button 
                                ...
                                className="flex items-center justify-center px-5 py-2.5 text-white bg-gradient-to-r from-purple-500 to-[#167468] rounded-2xl border border-white/10 hover:shadow-lg hover:shadow-[#167468]/30 transition-all font-medium disabled:opacity-60" 
                            ...
                            {isLoggedIn ? (
                                <p className="text-sm text-gray-300 hidden lg:block"> <span className="font-semibold text-[#167468]">{currentUser.username}</span></p>
                            ) : (
                                <p className="text-sm text-gray-300 hidden lg:block">Log in to interact.</p>
                            )}
                        </div>
                    </header>
                    <div className="flex items-center text-sm text-gray-300 my-4 space-x-1">
                ...
                <div className="lg:col-span-2">
                    {error && (
                        <div className="p-4 mb-4 text-sm text-red-200 bg-white/5 border border-white/10 rounded-2xl" role="alert">
                            {error}
                        </div>
                    )}
                    {successMessage && (
                        <div className="p-4 mb-4 text-sm text-green-200 bg-white/5 border border-white/10 rounded-2xl" role="status">
                            {successMessage}
                        </div>
                    )}
                    ...
                    {isNewPostFormVisible && (
                        <div className="bg-white/5 border border-white/10 backdrop-blur p-6 rounded-2xl shadow-lg mb-6">
                            <div className="flex items-center justify-between mb-4 border-b pb-4 border-white/10">
                                <h2 className="text-xl font-semibold text-white">Create a New Post</h2>
                                <button onClick={toggleNewPostFormVisibility} className="flex items-center text-gray-300 hover:text-white transition-colors">
                            ...
                            <form onSubmit={handlePostSubmit} className="space-y-4">
                                <div>
                                    <input
                                        ...
                                        className="w-full px-4 py-2 rounded-2xl border border-white/10 bg-white/5 text-white placeholder:text-gray-400 focus:ring-2 focus:ring-[#167468] focus:border-transparent transition-colors"
                                        required
                                    />
                                </div>
                                <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="newPostCategory" className="block text-sm font-medium text-gray-200 mb-1">Category</label>
                                        <select
                                            id="newPostCategory"
                                            value={newPostCategory}
                                            onChange={(e) => setNewPostCategory(e.target.value)}
                                            className="w-full px-4 py-2 rounded-2xl border border-white/10 bg-white/5 text-white text-sm focus:ring-2 focus:ring-[#167468] focus:border-transparent transition-colors"
                                        >
                                    ...
                                    <div>
                                        <label htmlFor="newPostTags" className="block text-sm font-medium text-gray-200 mb-1">Tags (comma-separated)</label>
                                        <input
                                            id="newPostTags"
                                            type="text"
                                            value={newPostTags}
                                ...
                                <div>
                                    <label htmlFor="newPostImageInput" className="block text-sm font-medium text-gray-200 mb-1">Attach Image URL (Optional)</label>
                                    <input
                                ...
                                <button
                                    type="submit"
                                    className="w-full flex items-center justify-center px-5 py-2.5 text-white bg-gradient-to-r from-purple-500 to-[#167468] rounded-2xl border border-white/10 hover:shadow-lg hover:shadow-[#167468]/30 transition-all font-medium disabled:opacity-60"
                        ...
                        <div className="flex items-center space-x-2 mb-4">
                            <div className="relative flex-grow">
                                <input
                                    ...
                                    className="w-full px-4 py-2 pl-10 rounded-2xl border border-white/10 bg-white/5 text-white placeholder:text-gray-400 focus:ring-2 focus:ring-[#167468] focus:border-transparent transition-colors"
                                />
                                <Search size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                            </div>
                            <button onClick={toggleAdvancedFilterExpanded} className="flex items-center justify-center p-2 rounded-2xl bg-white/5 border border-white/10 text-gray-200 hover:bg-white/10 transition-colors">
                                <Filter size={20} />
                            </button>
                        </div>
                        
                        <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                            <span className="truncate text-gray-400">DISPLAYING {pagedPosts.length} OUT OF {allPosts.filter(p => !p.parentId).length} DISCUSSIONS</span>
                            <div className="flex items-center space-x-2">
                                ...
                                <select
                                    ...
                                    className="px-3 py-1.5 rounded-2xl border border-white/10 bg-white/5 text-white text-sm"
                                >
                        ...
                        {isAdvancedFilterExpanded && (
                            <div className="mt-4 border-t pt-4 border-white/10">
                                <div className="space-y-4">
                                    <div>
                                        <label htmlFor="filter-tag" className="block text-sm font-medium text-gray-200 mb-1">Filter by Tag</label>
                                        <input
                                            id="filter-tag"
                                            type="text"
                                            value={filterTag}
                            ...
                            posts.length > 0 ? (
                                pagedPosts.map(post => (
                                    <Fragment key={post._id}>
                                        <div className={!post.parentId ? "bg-white/5 border border-white/10 backdrop-blur rounded-2xl shadow-lg" : "relative ml-8 sm:ml-16 before:content-[''] before:absolute before:top-0 before:left-[-24px] before:w-[2px] before:h-full before:bg-white/20"}>
                                            <Post
                                                post={post}
                                                onToggleReplies={handleToggleReplies}
                                        ...
                                        {replyingTo === post._id && (
                                            <div className="mt-4 p-4 ml-8 sm:ml-16 bg-white/5 border border-white/10 backdrop-blur rounded-2xl shadow-inner">
                                                <form onSubmit={(e) => handleReplySubmit(e, post._id)} className="space-y-4">
                                                    ...
                                                    <div className="flex justify-end space-x-2">
                                                        <button type="button" onClick={confirmCancelReply} className="flex items-center px-4 py-2 text-sm font-medium rounded-2xl text-gray-200 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                                                            <X size={16} className="mr-2" /> Cancel
                                                        </button>
                                                        <button
                                                            type="submit"
                                                            className="flex items-center px-4 py-2 text-sm font-medium rounded-2xl text-white bg-gradient-to-r from-purple-500 to-[#167468] border border-white/10 hover:shadow-lg hover:shadow-[#167468]/30 transition-all disabled:opacity-60"
                                                            disabled={isSubmitting || !isLoggedIn}
                                                        >
                                                            {isSubmitting ? (
                                                        ...
                                                        <>
                                                            {totalReplyPages > 1 && (
                                                                <div className="flex items-center justify-between gap-3 px-1">
                                                                    <span className="text-xs text-gray-400">Replies page {replyPage} / {totalReplyPages}</span>
                                                                    <div className="flex items-center gap-2">
                                                                        <button
                                                                            type="button"
                                                                            onClick={() => setReplyPages(prev => ({ ...prev, [post._id]: Math.max(1, (prev[post._id] || 1) - 1) }))}
                                                                            disabled={replyPage <= 1}
                                                                            className="px-3 py-1.5 rounded-2xl text-xs text-gray-200 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors disabled:opacity-50 disabled:hover:bg-white/5"
                                                                        >
                                                                            Previous
                                                                        </button>
                                                                        <button
                                                            ...
                                                            {pagedReplies.map(reply => (
                                                                <div key={reply._id} className="relative before:content-[''] before:absolute before:top-0 before:left-[-24px] before:w-[2px] before:h-full before:bg-white/20">
                                                                    <Post
                                                                        post={reply}
                                                                        onReply={startReply}
                                                                        onLike={handleLike}
                ...
                <div className="lg:col-span-1 space-y-6">
                    {/* Paid Courses Section */}
                        ...
                        <button
                            className="sidebar-toggle-button lg:hidden w-full text-left mb-2 p-3 bg-white/5 border border-white/10 backdrop-blur rounded-2xl text-gray-200"
                        ...
                        <div className={`${showPremiumMobile ? 'block' : 'hidden'} lg:block bg-white/5 border border-white/10 backdrop-blur p-6 rounded-2xl shadow-sm mb-6 text-center`}>
                                <h3 className="font-semibold text-white">Forum Premium</h3>
                                <p className="text-sm text-gray-300 mt-2">Want to post questions and replies without limits? Visit our Home page to subscribe to Forum Premium.</p>
                                <div className="mt-3">
                                    <ViewPlansButton />
                                </div>
                            </div>
                            ...
                            {paidCourses.length > 0 && (
                                <div className={`${showPaidCoursesMobile ? 'block' : 'hidden'} lg:block bg-white/5 border border-white/10 backdrop-blur p-6 rounded-2xl shadow-lg`}>
                                <h2 className="text-xl font-semibold mb-2 border-b pb-2 border-white/10 text-white">
                                    💎 Premium Courses
                                </h2>
                                ...
                                <div className="space-y-3 max-h-96 overflow-y-auto">
                                    {paidCourses.map(course => (
                                        <div 
                                            key={course._id} 
                                            className="p-3 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/10 transition-colors cursor-pointer group"
                                            ...
                                            <div className="flex items-start space-x-3">
                                                ...
                                                <div className="flex-grow min-w-0">
                                                    ...
                                                    <div className="flex items-center justify-between mt-1">
                                                        <span className="text-xs font-bold text-[#167468]">
                    ...
                    <button
                        className="lg:hidden w-full text-left mb-2 p-3 bg-white/5 border border-white/10 backdrop-blur rounded-2xl text-gray-200"
                    ...
                    <div className={`${showTopContributorsMobile ? 'block' : 'hidden'} lg:block bg-white/5 border border-white/10 backdrop-blur p-6 rounded-2xl shadow-lg`}>
                        <h2 className="text-xl font-semibold mb-2 border-b pb-2 border-white/10 text-white">Top Contributors</h2>
                        <p className="text-gray-300 text-sm mb-4">
                        ...
                        <ul className="space-y-4">
                            {topContributors.length > 0 ? (
                                ...
                                <li className="text-gray-400 text-sm text-center py-4">No contributors yet</li>
                            )}
                        </ul>
                    </div>
